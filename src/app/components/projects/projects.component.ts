...
export class ProjectsComponent implements OnInit {
  ...
  toggleFollow(project: any): void {
    if (project.is_following) {
      this.adminService.unfollowProject(project.id).subscribe({
        next: () => {
          project.is_following = 0;
          this.toastService.show('Unfollowed project', 'success');
        },
        error: (err) => {
      ...
      this.adminService.followProject(project.id).subscribe({
        next: () => {
          project.is_following = 1;
          this.toastService.show('Followed project', 'success');
        },
        error: (err) => {
