  ...
  try {
    ...
    let query = `
      SELECT DISTINCT p.*, 
             ...
             (SELECT COUNT(*) FROM project_followers pf WHERE pf.project_id = p.id AND pf.user_id = ?) as is_following
      FROM projects p 
...
async function createProject(req, res) {
  try {
    ...
    if (!["admin", "manager", "engineer", "head manager"].includes(userRole)) {
      return res
        ...
        .json({
          success: false,
          message:
            "Access denied. Only admins, managers, employees, and head managers can create projects.",
        });
    }
    ...
    if (creatorRole === "head manager") {
      try {
        ...
        const [existingRows] = await db.query(
          "SELECT * FROM head_manager_projects WHERE project_id = ?",
          [projectId]
        );
        if (existingRows.length === 0) {
          // Auto-select the project for the head manager who created it
          await db.query(
            "INSERT INTO head_manager_projects (head_manager_id, project_id) VALUES (?, ?)",
            [req.user.id, projectId]
          );
          console.log(
            `Auto-selected project ${projectId} for head manager ${req.user.id}`
          );
        }
      } catch (selectError) {
        console.error(
          "Error auto-selecting project for head manager:",
          selectError
        );
    ...
    if (normalizedCustomerId) {
      try {
        await db.query("UPDATE customers SET project_name = ? WHERE id = ?", [
          name.trim(),
          normalizedCustomerId,
        ]);
        console.log(
          `Updated customer ${normalizedCustomerId} project_name to "${name}"`
        );
      } catch (customerUpdateError) {
        console.error(
          "Error updating customer project_name:",
          customerUpdateError
        );
    ...
    if (creatorRole === "engineer") {
      try {
        ...
        console.log(
          `Project creation notification sent to admins: ${projectMessage}`
        );

        // Notify all managers
        await Notification.notifyAllManagers(projectMessage, "project_created");
        console.log(
          `Project creation notification sent to managers: ${projectMessage}`
        );

        // Notify all head managers
        await Notification.notifyAllHeadManagers(
          projectMessage,
          "project_created"
        );
        console.log(
          `Project creation notification sent to head managers: ${projectMessage}`
        );
      } catch (notifError) {
        console.error(
          "Error sending notifications for employee project creation:",
          notifError
        );
        // Don't fail the request if notification creation fails
      }
      ...
      try {
        ...
        console.error(
          "Error sending admin notification for project creation:",
          notifError
        );
        // Don't fail the request if notification creation fails
      }
    ...
    res.json({
      ...
      project: { id: result.insertId, name, description, start_date, end_date },
    });
  } catch (error) {
...
async function updateProject(req, res) {
  try {
    ...
    const updateQuery = `UPDATE projects SET ${updateFields
      .map((f) => `${f} = ?`)
      .join(", ")} WHERE id = ?`;
    ...
    const fieldsToTrack = {
      ...
      archived: {
        old: project.archived ? true : false,
        new:
          archived !== undefined ? archived : project.archived ? true : false,
      },
      customer_id: { old: project.customer_id, new: normalizedCustomerId },
      region: { old: project.region, new: projectRegion },
      allocated_time: {
        old: project.allocated_time,
        new: allocated_time || null,
      },
    ...
    // Notify project followers about the update
    try {
      const updaterName = userName || 'User';
      const notificationMessage = `Project "${name}" was updated by ${updaterName}`;
      await Notification.notifyProjectFollowers(id, notificationMessage, 'project_updated', req.user.id);
    } catch (error) {
    ...
    if (archived !== undefined) {
      try {
        ...
        if (oldArchivedStatus !== newArchivedStatus) {
          ...
          try {
            ...
            if (alterError.code !== "ER_DUP_FIELDNAME") {
              console.warn(
                "Warning: Could not ensure archived column exists:",
                alterError.message
              );
            }
          }
          ...
          const [updateResult] = await db.query(
            "UPDATE tasks SET archived = ? WHERE project_id = ?",
          ...
          if (updateResult.affectedRows > 0) {
            ...
            console.log(
              `ℹ️ No tasks found to update for project ${id} (project may have no tasks)`
            );
          }
        } else {
          console.log(
        ...
        console.error(
          "❌ Error archiving/unarchiving tasks:",
          taskArchiveError
        );
    ...
    if (normalizedCustomerId) {
      ...
      try {
        await db.query(
          "UPDATE customers SET project_name = NULL WHERE id = ?",
          [project.customer_id]
        );
        console.log(
          `Cleared project_name from customer ${project.customer_id}`
        );
      } catch (customerUpdateError) {
        console.error(
          "Error clearing customer project_name:",
          customerUpdateError
        );
        // Don't fail the project update if customer update fails
      }
    ...
    if (previousManagerId && previousManagerId !== normalizedManagerId) {
      ...
      try {
        console.log(
          `Creating removal notification for manager ${previousManagerId}: ${notificationMessage}`
        );
        ...
        console.log(
          `Removal notification created successfully with ID: ${notificationId}`
        );
      } catch (notifError) {
      ...
      try {
        ...
        const managerName =
          managerRows.length > 0 ? managerRows[0].name : "Unknown";
        ...
        console.error(
          "Error sending admin notification for manager removal:",
          notifError
        );
      }
    }
    ...
    if (normalizedManagerId && normalizedManagerId !== previousManagerId) {
      ...
      try {
        console.log(
          `Creating notification for manager ${normalizedManagerId}: ${notificationMessage}`
        );
        const notificationId = await Notification.createNotification(
          normalizedManagerId,
          notificationMessage,
      ...
      try {
        ...
        if (employeeRows.length > 0) {
          ...
          const managerName =
            managerRows.length > 0 ? managerRows[0].name : "Your Manager";
          ...
          for (const employee of employeeRows) {
            try {
              ...
              console.log(
                `Project assignment notification sent to employee ${employee.id} (${employee.email}): ${employeeNotificationMessage}`
              );
            } catch (empNotifError) {
              console.error(
                `Error sending notification to employee ${employee.id}:`,
                empNotifError
              );
        ...
        console.error(
          "Error notifying employees about project assignment:",
          empNotifError
        );
        // Don't fail the request if notification creation fails
      }
      ...
      try {
        ...
        const managerEmail =
          managerRows.length > 0 ? managerRows[0].email : "Unknown";
        ...
        console.error(
          "Error sending admin notification for manager assignment:",
          notifError
        );
      }
    }
    ...
    if (userRole === "engineer") {
      try {
        ...
        const managerId =
          employeeRows.length > 0 && employeeRows[0].manager_id
            ? employeeRows[0].manager_id
            : null;
        ...
        if (updateMessage) {
          ...
          console.log(
            `Project update notification sent to admins: ${updateMessage}`
          );
          ...
          if (managerId) {
            try {
              ...
              console.log(
                `Project update notification sent to employee's manager (ID: ${managerId}): ${updateMessage}`
              );

              // Notify head managers for this manager
              await Notification.notifyHeadManagersForManager(
                managerId,
                updateMessage,
                "project_updated"
              );
              console.log(
                `Project update notification sent to head managers for manager ${managerId}: ${updateMessage}`
              );
            } catch (managerNotifError) {
              console.error(
                "Error sending notification to employee's manager:",
                managerNotifError
              );
            }
          }
        ...
        console.error(
          "Error sending notifications for employee project update:",
          notifError
        );
        // Don't fail the request if notification creation fails
      }
...
async function deleteProject(req, res) {
  try {
    ...
    const [projectRows] = await db.query(
      "SELECT name, customer_id FROM projects WHERE id = ?",
      [id]
    );
    if (projectRows.length === 0) {
      return res
    ...
    if (customerId) {
      try {
        ...
        console.log(
          `Cleared project_name from customer ${customerId} after project deletion`
        );
      } catch (customerUpdateError) {
        console.error(
          "Error clearing customer project_name:",
    ...
    try {
      ...
      console.error(
        "Error sending admin notification for project deletion:",
        notifError
      );
      // Don't fail the request if notification creation fails
    }
...
async function assignManagerToProject(req, res) {
  try {
    ...
    if (userRole !== "head manager" && userRole !== "admin") {
      return res
        ...
        .json({
          success: false,
          message:
            "Access denied. Only head managers and admins can assign managers to projects.",
        });
    }
    ...
    await db.query("UPDATE projects SET manager_id = ? WHERE id = ?", [
      normalizedManagerId,
      id,
    ]);

    // Get head manager name
    const [headManagerRows] = await db.query(
      "SELECT name FROM users WHERE id = ?",
      [req.user.id]
    );
    const headManagerName =
      headManagerRows.length > 0 ? headManagerRows[0].name : "Head Manager";
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to assign manager to project" });
  }
}
  ...
  try {
    ...
    if (userRole !== "head manager") {
      return res
        ...
        .json({
          success: false,
          message: "Access denied. Only head managers can select projects.",
        });
    }
    ...
    if (existingRows.length > 0) {
      ...
      if (existingHeadManagerId === headManagerId) {
        await db.query(
          "DELETE FROM head_manager_projects WHERE project_id = ? AND head_manager_id = ?",
          [id, headManagerId]
        );
        ...
        try {
          // Get head manager name
          const [headManagerNameRows] = await db.query(
            "SELECT name FROM users WHERE id = ?",
            [headManagerId]
          );
          const headManagerName =
            headManagerNameRows.length > 0
              ? headManagerNameRows[0].name
              : "Head Manager";
          const project = projectRows[0];
          ...
          console.error(
            "Error sending admin notification for project deselection:",
            notifError
          );
          // Don't fail the request if notification creation fails
        }
        ...
        const [headManagerRows] = await db.query(
          "SELECT name FROM users WHERE id = ?",
          [existingHeadManagerId]
        );
        const headManagerName =
          headManagerRows.length > 0
            ? headManagerRows[0].name
    ...
    try {
      ...
      console.error(
        "Error sending admin notification for project selection:",
        notifError
      );
      // Don't fail the request if notification creation fails
    }
    ...
    if (error.code === "ER_DUP_ENTRY") {
      return res
        .status(400)
        .json({ success: false, message: "This project already Selected" });
    }
    res
      .status(500)
      .json({ success: false, message: "Failed to select project" });
  }
}
...
async function getSelectedProjects(req, res) {
  try {
    ...
    if (userRole !== "head manager") {
      return res
        ...
        .json({
          success: false,
          message:
            "Access denied. Only head managers can view selected projects.",
        });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch selected projects" });
  }
}
...
async function getProjectComments(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch project comments" });
  }
}
...
async function updateProjectComment(req, res) {
  try {
    ...
    // Update comment
    await db.query("UPDATE project_comments SET comment = ? WHERE id = ?", [
      comment.trim(),
      commentId,
    ]);
...
async function followProject(req, res) {
  try {
    ...
    const result = await Project.addFollower(projectId, userId);
    if (!result) {
      return res.json({ success: true, message: "Already following project" });
    }
...
async function unfollowProject(req, res) {
  try {
    ...
    await Project.removeFollower(projectId, userId);
...
async function getProjectFollowers(req, res) {
  try {
    const projectId = req.params.id;
    const followers = await Project.getProjectFollowers(projectId);

    // Check if current user is following
    const isFollowing = await Project.isFollowing(projectId, req.user.id);

    res.json({ success: true, followers, isFollowing });
  } catch (error) {
    console.error("Error fetching project followers:", error);
    res.status(500).json({ success: false, message: "Failed to fetch project followers" });
  }
}
