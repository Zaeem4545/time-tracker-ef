...
async function createTask(req, res) {
  try {
    ...
    // Notify project followers about task creation
    try {
      // Get project name
      const [projectRows] = await db.query('SELECT name FROM projects WHERE id = ?', [project_id]);
      const projectName = projectRows.length > 0 ? projectRows[0].name : 'a project';
      ...
      const notificationMessage = `Task "${title}" in project "${projectName}" was created by ${creatorName}`;
      await Notification.notifyProjectFollowers(parseInt(project_id), notificationMessage, 'task_created', req.user.id);
    } catch (notifError) {
      console.error('Error sending project follower notification for task creation:', notifError);
    }
...
async function updateTask(req, res) {
  try {
    ...
    // Notify project followers about task update
    try {
      const [projectRows] = await db.query('SELECT name FROM projects WHERE id = ?', [currentTask.project_id]);
      const projectName = projectRows.length > 0 ? projectRows[0].name : 'a project';
      ...
      const notificationMessage = `Task "${title}" in project "${projectName}" was updated by ${updaterName}`;
      await Notification.notifyProjectFollowers(currentTask.project_id, notificationMessage, 'task_updated', req.user.id);
    } catch (error) {
      console.error('Error notifying project followers for task update:', error);
    }
...
async function deleteTask(req, res) {
  try {
    ...
    // Notify project followers about task deletion
    if (currentTask) {
      try {
        const [updaterRows] = await db.query('SELECT name FROM users WHERE id = ?', [req.user.id]);
        const updaterName = updaterRows.length > 0 ? updaterRows[0].name : 'User';
        const notificationMessage = `Task "${currentTask.title}" was deleted by ${updaterName}`;
        await Notification.notifyProjectFollowers(currentTask.project_id, notificationMessage, 'task_deleted', req.user.id);
      } catch (error) {
        console.error('Error notifying project followers for deletion:', error);
      }
    }
